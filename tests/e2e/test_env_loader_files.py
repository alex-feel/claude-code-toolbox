...
class TestEnvLoaderFileGeneration:
    ...
    def test_header_only_rewrite_when_all_deletions(
        self,
        e2e_isolated_home: dict[str, Path],
    ) -> None:
        """Verify loader files are rewritten header-only when all vars are deletions.

        A stale loader file from a prior run must stop exporting deleted
        variables, otherwise the launcher re-applies them at session start.
        """
        paths = e2e_isolated_home
        ...
        (cmd_dir / 'env.sh').write_text(stale, newline='\n')

        result = generate_env_loader_files(all_deletions, ['test-cmd'], cmd_dir)

        assert result, 'Expected loader files to be rewritten for all-deletion vars'
        content = (cmd_dir / 'env.sh').read_text(encoding='utf-8')
        assert content.startswith('# Auto-generated by claude-code-toolbox')
        assert 'DEL_A' not in content
        assert 'DEL_B' not in content
