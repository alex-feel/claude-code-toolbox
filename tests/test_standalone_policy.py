...
class TestAutoUpdateConstantsParity:
    """Enforce that auto-update management constants are identical in both scripts."""

    def test_auto_update_key_identical(self) -> None:
        """AUTO_UPDATE_KEY must be identical in both scripts."""
        ic_source = _read_source(INSTALL_CLAUDE)
        se_source = _read_source(SETUP_ENVIRONMENT)

        ic_match = re.search(r"AUTO_UPDATE_KEY\s*=\s*['\"](.+?)['\"]", ic_source)
        se_match = re.search(r"AUTO_UPDATE_KEY\s*=\s*['\"](.+?)['\"]", se_source)

        assert ic_match is not None, 'install_claude.py must define AUTO_UPDATE_KEY'
        assert se_match is not None, 'setup_environment.py must define AUTO_UPDATE_KEY'
        assert ic_match.group(1) == se_match.group(1), (
            f'AUTO_UPDATE_KEY diverged: install_claude={ic_match.group(1)!r} '
            f'vs setup_environment={se_match.group(1)!r}'
        )

    def test_auto_update_disabled_value_identical(self) -> None:
        """AUTO_UPDATE_DISABLED_VALUE must be identical in both scripts (False)."""
        ic_source = _read_source(INSTALL_CLAUDE)
        se_source = _read_source(SETUP_ENVIRONMENT)

        ic_match = re.search(r'AUTO_UPDATE_DISABLED_VALUE\s*=\s*(True|False)', ic_source)
        se_match = re.search(r'AUTO_UPDATE_DISABLED_VALUE\s*=\s*(True|False)', se_source)

        assert ic_match is not None, 'install_claude.py must define AUTO_UPDATE_DISABLED_VALUE'
        assert se_match is not None, 'setup_environment.py must define AUTO_UPDATE_DISABLED_VALUE'
        assert ic_match.group(1) == se_match.group(1), (
            f'AUTO_UPDATE_DISABLED_VALUE diverged: install_claude={ic_match.group(1)!r} '
            f'vs setup_environment={se_match.group(1)!r}'
        )

    def test_disable_autoupdater_key_identical(self) -> None:
        """DISABLE_AUTOUPDATER_KEY must be identical in both scripts."""
        ic_source = _read_source(INSTALL_CLAUDE)
        se_source = _read_source(SETUP_ENVIRONMENT)

        ic_match = re.search(r"DISABLE_AUTOUPDATER_KEY\s*=\s*['\"](.+?)['\"]", ic_source)
        se_match = re.search(r"DISABLE_AUTOUPDATER_KEY\s*=\s*['\"](.+?)['\"]", se_source)

        assert ic_match is not None, 'install_claude.py must define DISABLE_AUTOUPDATER_KEY'
        assert se_match is not None, 'setup_environment.py must define DISABLE_AUTOUPDATER_KEY'
        assert ic_match.group(1) == se_match.group(1), (
            f'DISABLE_AUTOUPDATER_KEY diverged: install_claude={ic_match.group(1)!r} '
            f'vs setup_environment={se_match.group(1)!r}'
        )

    def test_disable_autoupdater_value_identical(self) -> None:
        """DISABLE_AUTOUPDATER_VALUE must be identical in both scripts (string "1")."""
        ic_source = _read_source(INSTALL_CLAUDE)
        se_source = _read_source(SETUP_ENVIRONMENT)

        ic_match = re.search(r"DISABLE_AUTOUPDATER_VALUE\s*=\s*['\"](.+?)['\"]", ic_source)
        se_match = re.search(r"DISABLE_AUTOUPDATER_VALUE\s*=\s*['\"](.+?)['\"]", se_source)

        assert ic_match is not None, 'install_claude.py must define DISABLE_AUTOUPDATER_VALUE'
        assert se_match is not None, 'setup_environment.py must define DISABLE_AUTOUPDATER_VALUE'
        assert ic_match.group(1) == se_match.group(1), (
            f'DISABLE_AUTOUPDATER_VALUE diverged: install_claude={ic_match.group(1)!r} '
            f'vs setup_environment={se_match.group(1)!r}'
        )
