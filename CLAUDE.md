...
The standalone Python scripts, the `cc-toolbox` CLI, and the platform bootstrap wrappers expose ONE interface: identical flags, identical environment variables, identical behavior. The wrappers forward ALL user arguments to `setup_environment.py` verbatim (consuming only the config positional when it is supplied as the first argument) -- no allowlists, no wrapper-built flags. Anything expressible in one entry path MUST be expressible identically in every other, and any interface change lands in all entry paths (Python scripts, `cli.py`, all three wrappers, docs) in the same PR.
