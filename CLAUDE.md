...
**Strict-mode scope warning:** the launcher enables `--strict-mcp-config --mcp-config mcp.json` by a runtime file-existence test, so as soon as a run produces profile-scoped servers the isolated sessions load ONLY that file. `configure_all_mcp_servers(..., command_names=...)` collects every server whose normalized scopes are non-profile-only into `strict_hidden` during the loop and, after it (once `profile_servers` is known to be non-empty and the run is isolated), emits one `warning()` per server naming the server, its scope, every command name sharing the launcher, and the `scope: [<scope>, profile]` fix. `stats['strict_hidden_count']` carries the count into the completion summary. `normalize_scope()` forbids combining two non-profile scopes, so the named scope is always singular.
