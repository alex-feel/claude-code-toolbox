...
**Machine-wide pin gate:** The machine has ONE Claude Code binary, so its auto-update and IDE-extension controls are machine-global and shared by every installed profile. Removal is therefore gated on `machine_pinned = is_pinned or other_profile_pinned`, computed once in `main()` before `apply_auto_update_settings()`. `_other_profile_pins(home_dir, primary_command_name)` builds `other_profile_pinned` by reading `~/.claude/manifest.json` plus every `~/.claude/*/manifest.json` via `_read_profile_manifest()` (missing file, unreadable file, and non-object JSON are all tolerated as "no pin"), skipping the manifest whose `name` equals this run's `primary_command_name` (`None` identifies the base profile, so a profile relocated by a user-set `CLAUDE_CONFIG_DIR` is still matched as its own), and returning the sorted display names of the rest that record a non-empty `claude_code_version`. When this run is unpinned and another profile pins, `main()` prints an info line naming those profiles. `machine_pinned` threads into `_run_stale_controls_cleanup()`; `other_profile_pinned` threads into `apply_auto_update_settings()` / `apply_ide_extension_settings()` as a keyword-only argument.
