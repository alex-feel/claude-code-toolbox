...
An isolated command exports `CLAUDE_CONFIG_DIR` for the session it launches, so a setup run started from a terminal inside that session inherits the variable; a configuration can also persist it through `os-env-variables` or a `user-settings.env` entry, in which case every terminal carries it. The Claude CLI resolves `CLAUDE_CONFIG_DIR` ahead of the home directory and offers no fallback, so the inherited value decides where `claude mcp add` and the global-config writes land. Any non-empty value counts, matching the CLI. The setup checks the variable as soon as it knows the directory the run targets -- before the component picker, the Windows elevation prompt and the installation summary -- so no prompt precedes the outcome and `--dry-run` reports it too:
